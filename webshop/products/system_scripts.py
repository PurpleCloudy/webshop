    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            for brand in form.cleaned_data['brands'].split(','):
                models.Brand.objects.create(name=brand, slug=slugify(brand),
            ...
            for seller in form.cleaned_data['sellers'].split(','):
                models.Seller.objects.create(name=seller, slug=slugify(seller),
                                             preview='path_to_static/no_image_default.jpg',
                                             description='asddaksdjaksjdkasjdkajsdkjijihuhiuya',
                                             official_name=seller,
                                             licence=True)
            for product in form.cleaned_data['products'].split(','):
                prod = models.Product(category=models.Category.objects.get(pk=randint(1, 8)),
                                      brand=models.Brand.objects.get(pk=randint(1992, 2008)),
                                      seller=models.Seller.objects.get(pk=randint(2640, 2649)),
                                      name=product,
